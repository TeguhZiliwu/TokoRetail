...
if (!empty($userLogin)) {
    try {
        ...
        $query = ("SELECT cctvrecordid, cctvrecordname, remark, createdby, createddate, updatedby, updateddate FROM tcctvrecord WHERE (remark LIKE ?)");
        ...
        if ($result->num_rows > 0) {
            while ($data = $result->fetch_assoc()) {
                $data = array(
                    'CCTVRecordID' => $data['cctvrecordid'],
                    'RecordName' => $data['cctvrecordname'],
                    'Remark' => $data['remark'],
